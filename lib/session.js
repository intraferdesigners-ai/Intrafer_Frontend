...
// popup after a 5-minute cooldown), this is a permanent one-shot flag —
// once a visitor dismisses or submits for a given vendor, that vendor's
// overlay never reappears for them again.
...
// Guest contact details (name/phone/email), remembered across vendors so a
// visitor never has to retype them on a second enquiry in the same browser.
// OTP verification still happens on every enquiry regardless — this only
// skips retyping the contact fields, not re-verification.
...
export function getSavedContact() {
  ...
  try {
    ...
    if (!saved || !saved.name || !saved.phone || !saved.email) return null;
    return saved;
  } catch {
    return null;
  }
...
export function saveContact({ name, phone, email }) {
  if (typeof window === 'undefined') return;
  localStorage.setItem(SAVED_CONTACT_KEY, JSON.stringify({ name, phone, email }));
}
