...
// popup after a 5-minute cooldown), this only a real submission sets it
// (VendorEnquiryOverlay's handleSubmit). Dismissing without submitting does
// NOT call markVendorEngaged: the overlay is meant to keep reappearing on
// every fresh visit to that vendor's page until the visitor actually
// submits an enquiry there, not be permanently suppressed by closing it
// once.
//
// Expires after 24h rather than being permanent — matches the backend's own
// dedup window in enquiry.controller.js's autoSubmitEnquiry. This used to be
// a one-shot-forever flag, which meant the exact same visitor genuinely
// wanting to reach out to the same vendor again days later (a second,
// unrelated project) had that resubmission silently swallowed with no error
// on either side. Keeping this in sync with the backend window means a
// same-browser retry after 24h actually reaches the server instead of being
// blocked client-side before the request is even made.
const ENGAGED_VENDORS_KEY = 'intrafer_vendor_engaged';
const ENGAGED_VENDOR_WINDOW_MS = 24 * 60 * 60 * 1000;
...
export function hasEngagedVendor(vendorId) {
  ...
  const entry = readEngagedVendors()[vendorId];
  if (!entry) return false;
  return (Date.now() - entry.at) < ENGAGED_VENDOR_WINDOW_MS;
}
