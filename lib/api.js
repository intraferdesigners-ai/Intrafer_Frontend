...
const AUTH_FLOW_ENDPOINTS = [
  '/auth/login', '/auth/register', '/auth/google', '/auth/send-otp', '/auth/verify-otp', '/auth/refresh',
  // Guest enquiry OTP — unauthenticated by design (see enquiry.controller.js),
  // so a 401 here is never a dead session to recover from.
  '/enquiry/send-otp', '/enquiry/submit',
  // Silent auto-send with a rejected contactToken (see lib/enquiryFlow.js) —
  // same reasoning, and this one has no session/login screen to redirect to
  // even if it were treated as one.
  '/enquiry/auto-submit',
];
