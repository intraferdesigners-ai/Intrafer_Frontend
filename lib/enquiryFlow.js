...
export async function autoSendVendorEnquiry(vendor) {
  ...
  try {
    ...
    // Every failure path here used to be entirely silent — no console
    // signal, no toast (there's no UI to put one on, since this call has no
    // popup by design). Callers now fall back to showing the manual form on
    // any null return, but this log is what makes a failure diagnosable
    // from the browser console before it gets that far.
    console.warn('[autoSendVendorEnquiry] silent lead send failed, falling back to visible form:', err?.response?.status, err?.response?.data?.message || err?.message);
    return null;
  }
}
